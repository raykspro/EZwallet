...
export default function AddTransactionDialog() {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-end p-4">
      <div className="bg-slate-900 w-full max-w-md mx-auto rounded-t-[32px] p-8 border border-slate-800 shadow-2xl">
        <div className="flex justify-between items-center mb-6 text-white font-bold">
          <span>Novo Lançamento</span>
          <button onClick={() => setOpen(false)}><X className="text-slate-500" /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <select className="w-full h-12 bg-slate-800 rounded-xl px-4 text-white text-sm outline-none" value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})}>
            {formData.type === 'expense' ? GASTOS.map(c => <option key={c} value={c}>{c}</option>) : <option value="Salário">Salário</option>}
          </select>
          <select className="w-full h-12 bg-slate-800 rounded-xl px-4 text-white text-sm outline-none" value={formData.paymentMethod} onChange={(e) => setFormData({...formData, paymentMethod: e.target.value})}>
            {METODOS.map(m => <option key={m} value={m}>{m}</option>)}
          </select>
          <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-blue-900/20">Confirmar Lançamento</button>
        </form>
      </div>
    </div>
  );
