...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-24 text-slate-50">
      {/* Cabeçalho Nexus Finanças - Estrutura Superior */}
      <header className="fixed top-0 left-0 right-0 bg-slate-950/80 backdrop-blur-md z-40 max-w-md mx-auto p-6 flex items-center justify-between border-b border-slate-900">
        <div className="flex items-center gap-3">
          {/* A imagem logo-nexus.png deve estar na sua pasta public/ */}
          <img 
            src="/EZwallet/logo-nexus.png" 
      ...
      {/* Conteúdo Principal com espaçamento para o cabeçalho fixo */}
      <main className="max-w-md mx-auto p-6 pt-28">
        {children}
      </main>

      {/* Navegação Inferior de Comando */}
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-900/90 backdrop-blur-lg border-t border-slate-800 px-6 py-3 flex justify-around items-center max-w-md mx-auto z-50 rounded-t-[24px]">
        <Link href="/">
          <div className={`flex flex-col items-center gap-1 cursor-pointer transition-all ${isActive('/') ? 'text-blue-400 scale-110' : 'text-slate-500 hover:text-slate-300'}`}>
            <LayoutDashboard className="w-6 h-6" />
            <span className="text-[10px] font-black uppercase tracking-widest">Dashboard</span>
          </div>
        </Link>
        <Link href="/transactions">
          <div className={`flex flex-col items-center gap-1 cursor-pointer transition-all ${isActive('/transactions') ? 'text-blue-400 scale-110' : 'text-slate-500 hover:text-slate-300'}`}>
